...
A complete enterprise mirror should set `dist_server` and publish both `versions.json` and `nightly.json` at its root. You can also point `manifest_url` directly at an internal `versions.json`; cjv locates the nightly file in the same directory. See [Configuration](configuration.md).
