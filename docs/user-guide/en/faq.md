...
## Where do nightly versions and download URLs come from?

The default `manifest_url` points to `versions.json` generated by [`cangjie-version-manifest`](https://github.com/Zxilly/cangjie-version-manifest). That project periodically collects GitCode [`nightly_build`](https://gitcode.com/Cangjie/nightly_build/releases) releases and records nightly latest, history, platform SDKs, and component URLs in the manifest. `cjv install nightly`, `cjv update nightly`, `cjv check`, and remote listing commands all read this static data.

When a nightly SDK entry contains a SHA-256, cjv verifies it directly. When the entry awaits a checksum, cjv reads the asset's `<url>.sha256` sidecar. If upstream has not published the sidecar yet, cjv displays an integrity notice and relies on HTTPS transport protection. Enterprise deployments can populate SHA-256 for every approved artifact in their internal manifest. See [Internal distribution source](enterprise/distribution-server.md).
...
A complete enterprise mirror should set `dist_server`, making LTS, STS, nightly, and components share `<root>/versions.json`. You can also set `manifest_url` directly to the complete URL of an internal manifest. See [Configuration](configuration.md).
