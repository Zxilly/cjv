...
Unit tests live in the same package as the code under test, with file names ending in `_test.go`, scattered throughout `internal/`, currently over a hundred files. They use the `assert` and `require` helpers from `github.com/stretchr/testify` for assertions and do not depend on the network: when a distribution source is needed, they use the mock servers in `internal/testutil` (`MockDistServer`, `ValidMockServer`, `MockServerWithTargetSDKs`, `SplitNightlyMockServer` and friends) to spin up an `httptest.Server` that returns a constructed manifest and a packaged mock SDK; the `cli` and `lifecycle` tests share this set.
...
- `cli`, `cli/settings`, and `cli/selfmgmt` construct independent commands and parse real arguments, checking that repeated invocations do not share JSON mode, directory flags, or uninstall confirmation. Repeated installation of an existing SDK succeeds in both text and JSON modes, and component addition emits a complete JSON result. For `install` and `toolchain link`, `cli` tests only flag parsing, argument validation and result rendering; the installation behaviour itself is tested in `lifecycle`.
- `config` and `cli/settings` exercise real setting commands to verify that unrelated edits preserve fallback inheritance, an equal effective value can be pinned explicitly, and false, empty strings, and snapshot restoration keep their meaning. Invalid settings documents are rejected before publication, leaving the existing file and cache intact.
- `env` passes `Runtime` a base environment that differs from the process environment. Tests verify consistent path merging in child environments, contributions, and shell scripts, including platform variable names, casing, and isolation between repeated constructions.
- `lifecycle`: `install_test.go` drives `Install` to cover channel, bare-version and explicit-version resolution, target variants pinned to the host version, components landing under the target variant, the manifest being fetched once, the first install setting the default and bootstrapping the managed binary, validation failing without a compiler, and `Distribution.Resolve` picking the latest version available for a tuple; `resolved_install_test.go` verifies, for the manifest, URL and local-archive origins alike, that the one placement pipeline restores the old installation after a failed final step and propagates rollback errors; `link_test.go` tests only what each origin adds: the CI bundle with its inner stdx, cross-OS rejection, the local archive never being deleted, and the directory link referencing in place and being removed when finalization fails; `component_install_test.go` runs the production install path against an `httptest` server that serves component artifacts, covering input validation, target tuple selection and rollback of component batches, plus the proxy path resolving a channel name to the installed toolchain and reporting progress to the caller's sink. Progress assertions compare the event kinds a `testutil.ProgressRecorder` collected, never message text. Upgrade and removal tests cover SDK/stdx/docs content and settings references, components matching the new version, linked sources, existing replacement choices, failed replacement cleanup, and recovery errors. Components remain removable after reinstalling the SDK under the same name. Update tests in `update_test.go` cover the three update forms (channel name, target variant, explicit version), up-to-date and skipped-custom outcomes, a full update continuing past one failure, and the downloads purge; `cli` only tests the `update` command's argument handling, self-update decision and JSON rendering. The `init` and `component add` tests in `cli` also take the production path against the mock servers; there is no stand-in for the component installer.
- `progress` verifies that the `Text` adapter writes i18n text in event order and draws no download bar off a terminal, and that `Discard` is silent; the resume test in `dist` asserts that download events start at the resumed offset and add up to the full size; `cli/output` tests cover error output in both modes, rendering a partial success, and the choice of progress adapter.
- `component` verifies file and manifest restoration after failures in archive installation, local linking, and component batches. If restoration itself fails, the backup remains available for recovery. Component downloads go through the production `InstallFromSource`; tests serve a manifest over `httptest`, resolve it through a real `dist.Source`, and assert which artifact links were actually requested.
- `fstx` tests interrupted and repeated journal recovery, committed and prepared-for-publication states, and retained backups after blocked recovery. Invalid journals and paths outside the transaction scope must not modify unowned files.
- `reachable` unit-tests the PATH policy and the reachability operation. On Unix-like systems the tests point `HOME` at a temporary directory and verify that `ConfigurePath` writes the marker block once per shell config, writes nothing under `CJV_NO_PATH_SETUP=1`, that `RemovePath` undoes it, and that `Ensure` configures PATH only when the policy asks. On Windows the same checks run against the registry under `testutil.SaveRegistryPath`, on CI only. `Ensure` itself is tested for keeping or force-replacing the managed binary, the proxy links, and writing env scripts on request. `lifecycle` additionally verifies that publishing the first default toolchain configures PATH according to `Options.ConfigurePath` and that later installs leave it alone.
...
- `fsops` verifies the tree operations `MoveTree`/`CopyTree` on temp directories: nested directories, replacing and merging existing destination entries, keeping internal relative symlinks, rejecting tree-escaping symlinks without deleting the existing destination, and the cross-filesystem copy fallback triggered through `/dev/shm` on Linux. Windows-only tests cover junctions and the classification and remove retry for transient errors.
