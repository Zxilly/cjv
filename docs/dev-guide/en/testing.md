...
- `lifecycle` uses production installation entry points and real archives from a local test server to verify restoration for first installs, reinstalls, and URL installs. Upgrade and removal tests cover SDK/stdx/docs content and settings references, components matching the new version, linked sources, existing replacement choices, failed replacement cleanup, and recovery errors. Components remain removable after reinstalling the SDK under the same name. Update tests in `update_test.go` cover the three update forms (channel name, target variant, explicit version), up-to-date and skipped-custom outcomes, a full update continuing past one failure, and the downloads purge; `cli` only tests the `update` command's argument handling, self-update decision and JSON rendering.
