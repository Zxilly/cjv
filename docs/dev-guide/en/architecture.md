...
`internal/lifecycle` orchestrates download, extraction, verification, components, PATH configuration, the managed binary, and proxy links as one installation flow. Verification, the managed binary (`selfupdate.EnsureManagedExecutable`) and the proxy links (`sdktools.CreateAllProxyLinks`) are direct calls, not adapters; `Options` holds only what presentation and tests need: `Report`, `EnsurePathConfigured` and `ComponentInstall`. Dependencies point from `cli` toward `lifecycle`, and from `lifecycle` toward `selfupdate` and `sdktools`. `Report` carries progress and leaves operations silent when unset; the CLI owns output formatting. The same flow serves `cli install` and proxy auto-install: both leave the toolchain with the managed binary and proxy links in place. Reinstalling an existing SDK without force succeeds in both text and JSON modes, and the CLI also renders the JSON result of `component add`.
...
The process starts in `run` in `cmd/cjv/main.go`: `logging.Init` sets up logging, the program name is `cjv` rather than some tool name, so it takes the `cli.Execute` path. cobra routes the `install` subcommand to `runInstall` in `internal/cli/install.go`. `runInstall` collects the `--target`, `--component`, `--force` and other flags, assembles a `lifecycle.Options` (wiring in progress reporting and PATH configuration), and calls into `internal/lifecycle`.

`lifecycle` first asks `dist.Source` to resolve the requested channel, version, platform, and component artifacts from the manifest. The shared download and extraction path then materializes a staging directory, after which `component`, `selfupdate`, `sdktools`, and `fstx` complete component installation, the managed binary, proxy links, and transactional replacement. Every channel shares this installation path. The CLI selects progress reporting for its output mode and renders the command result through the invocation's renderer. `cli.Execute` renders errors, and `main` translates them into exit codes.
