...
`internal/dist` owns distribution sources and network artifacts. `source.go` is the unified entry point: LTS/STS lazily cache `versions.json`, while nightly lazily caches sibling `nightly.json`. Under `dist_server`, both live at the distribution root. Relative URLs resolve against the manifest directory and absolute URLs are honored as written. Component artifacts are resolved here as well, by channel, version and stdx platform (`ResolveComponent`), and `component.InstallFromSource` is the only component download path. `manifest.go` parses and validates channel data; `download.go` handles progress, retries, and SHA-256; `install.go` unpacks archives; `nightly.go` reads nightly SHA-256 sidecars. Host and target tuples are computed in `target` (`CurrentHostTuple`, `CurrentTargetTuple`); `dist` no longer forwards them.
