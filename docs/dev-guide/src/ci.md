...
4. 用 `Zxilly/upload-gitcode-release` 把 mirror 变体的产物(`dist/cjv-mirror_*` 和 `dist/checksums.txt`)单独传到 GitCode release。GitCode 仓库通过 Pull 镜像同步 GitHub 分支和 tag，Release 创建以镜像现有 `master` 为基点。
